...
// https://api.datamarket.azure.com/Bing/Search/v1/News?Query=%27xbox%27&$format=json&$skip=30
...
queue.process('topic', 1, function(job, cb) {
	...
	// request(URL+encodeURIComponent('"'+job.data.topic.get('name')+'"'), {timeout: 6000}, function(error, response, body) {
	...
	// 		return cb('Bad body response from google news... '+body);
	// 	}
...
function loadNewsPage(article, cb) {
	...
	request({
		...
		// 		return;
		// 	}
		// 	let scrubResult = scrub(contentData.raw);
		// 	if(typeof scrubResult !== 'string') {
		// 		return c.save({
		// 			error: 'Scrub returned '+scrubResult
		// 		});
		// 	}
		// 	else {
		// 		return c.save({
